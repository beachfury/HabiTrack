...
// =============================================================================
// POST /api/paid-chores/upload-image
// Upload a completion photo for a paid chore
// =============================================================================
export async function uploadPaidChoreImage(req: Request, res: Response) {
  const user = getUser(req);
  if (!user) {
    return authRequired(res);
  }
  ...
  try {
    ...
    const filename = `paidchore-${crypto.randomBytes(8).toString('hex')}.jpg`;
    const filepath = path.join(PAID_CHORES_DIR, filename);

    fs.writeFileSync(filepath, processed);

    const imageKey = `/uploads/paid-chores/${filename}`;

    log.info('Paid chore image uploaded', { userId: user.id, filename });
