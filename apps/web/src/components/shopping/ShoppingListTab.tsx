    ...
    <div className="space-y-3">
      ...
      {!hasItems ? (
        <div className="themed-card p-8 text-center">
          <ShoppingCart size={48} className="mx-auto mb-3 text-[var(--color-muted-foreground)] opacity-50" />
        ...
        >
          <div className="p-3 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm" style={{ color: 'color-mix(in srgb, var(--color-success) 80%, var(--color-foreground))' }}>
                {purchasedToday.length} items
              </p>
            </div>
          </div>
          <div className="border-t" style={{ borderColor: 'color-mix(in srgb, var(--color-success) 30%, transparent)' }}>
      ...
      {suggestions.length > 0 && (
        ...
        >
          <div className="p-3 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm" style={{ color: 'color-mix(in srgb, var(--color-primary) 80%, var(--color-foreground))' }}>
          ...
          <div className="border-t max-h-64 overflow-y-auto" style={{ borderColor: 'color-mix(in srgb, var(--color-primary) 30%, transparent)' }}>
            {suggestions.map((s) => (
              <div
                key={s.catalogItemId}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-[var(--color-foreground)] truncate">{s.itemName}</p>
                  <p className="text-xs text-[var(--color-muted-foreground)] truncate">{s.reason}</p>
                </div>
                {isAdmin && (
                  <button
